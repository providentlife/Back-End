...
export class WeightEntriesController {
    ...
    async getAllWeightEntriesFromOneSession(@Param('id', ParseIntPipe) id: number, @Res() res: Response) {
        ...
        if (allWeightEntries.length !== 0) return res.status(200).json({allWeightEntries: allWeightEntries});
        else return res.status(202).json({allWeightEntries: [], message: "No weight entries"});
    }
